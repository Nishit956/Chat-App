...
const LoginPage = () => {
  ...
  return (
    ...
    justify-center gap-2 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      {/* -------- left -------- */}
      <img src={assets.logo_big} alt="" className='w-[min(60vw,400px)]' />
      {/* -------- right -------- */}
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex
       flex-col gap-6 rounded-lg shadow-lg'>
        ...
        <button type='submit' className='py-3 bg-gradient-to-r from-cyan-400 to-cyan-600
        ...
        {currState === "Sign up" ? (
          <p className='text-sm text-gray-950'>Already have an account? <span
           onClick={()=>{setCurrState("Login"); setIsDataSubmitted(false)}}
          className='font-medium text-cyan-500 cursor-pointer'>Login here</span></p>
        ) : (
          <p className='text-sm text-gray-600'>Create an account <span
          onClick={()=>setCurrState("Sign up")}
          className='font-medium text-cyan-500 cursor-pointer'>Click here</span></p>
        )}
        </div>
       </form>
